...
class Players(object):
    ...
    def select_player(self, detailed=True, clear_player=False, header=get_localized(32042), combined=False):
        ...
        def _select_standard(players_list=None):
            ...
            for idx, i in enumerate(players_list):
                if 'auto_cloud' in str(i).lower() and self.tmdb_type != 'movie':
                    auto_var = idx
                    break
                if 'Auto_Torr_Scrape' in str(i) and self.tmdb_type == 'movie':
                    auto_var = idx
                    break
            #return Dialog().select(header, players, useDetails=detailed)
            #return Dialog().select(header, players, autoclose=30000, preselect=auto_var, useDetails=detailed)
            return Dialog().select(header, players, autoclose=30000, preselect=auto_var, useDetails=detailed)
