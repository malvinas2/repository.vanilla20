...
class SourceDetails():
    ...
    def getTitle():
        if SourceDetails.title is None:
            ...
            try:
                SourceDetails.title = SourceDetails.title
            except:
                pass
    ...
    def getTvShowTitle():
        if SourceDetails.tvshowtitle is None:
            ...
            # There are times when the title has a different encoding
            try:
                SourceDetails.tvshowtitle = SourceDetails.tvshowtitle
            except:
                pass
    ...
    def showList(self, exList):
        ...
        if select != -1:
            ...
            if (select == 0) and (addPlayAll is True):
                ...
                searchDetails = "/search/?q=%s+Extras" % urllib.parse.quote_plus(SourceDetails.getTitle().encode('utf8'))
...
class VideoExtrasWindow(xbmcgui.WindowXML):
    ...
    def createVideoExtrasWindow(files):
        return VideoExtrasWindow('script-videoextras-main.xml', ADDON.getAddonInfo('path'), 'default', '1080i', True, files=files)
    ...
    def onInit(self):
        ...
        if Settings.isYouTubeSearchSupportEnabled():
            ...
            li.setProperty("search", "/search/?q=%s+Extras" % urllib.parse.quote_plus(SourceDetails.getTitle().encode('utf8')))
            self.addItem(li)
...
class VideoExtrasContextMenu(xbmcgui.WindowXMLDialog):
    ...
    def createVideoExtrasContextMenu(extraItem):
        ...
        return VideoExtrasContextMenu("script-videoextras-context.xml", ADDON.getAddonInfo('path'), extraItem=extraItem)
...
class VideoPluginContextMenu(xbmcgui.WindowXMLDialog):
    ...
    def createYouTubeContextMenu(title):
        return VideoPluginContextMenu("script-videoextras-context.xml", ADDON.getAddonInfo('path'), pluginName='plugin.video.youtube', title=title)

    # Static method to create the Window Dialog class
    @staticmethod
    def createVimeoContextMenu(title):
        return VideoPluginContextMenu("script-videoextras-context.xml", ADDON.getAddonInfo('path'), pluginName='plugin.video.vimeo', title=title)
    ...
    def onClick(self, control):
        ...
        cmd = None
        escTitle = urllib.parse.quote_plus(self.title.encode('utf8'))
